...
// Marked as FCVAR_USERINFO so that the server can call CC messages before networking them down to us!!!
	...
	m_bPaintDebugInfo( false )
{
	...
	if ( !IsX360() )
	{
		g_pVGuiLocalize->AddFile( "resource/subtitles_%language%.txt", "GAME", true );
	}
	...
	char dbfile [ 512 ];
	Q_snprintf( dbfile, sizeof( dbfile ), "resource/subtitles_%s.dat", uilanguage );
...
void CHudCloseCaption::MsgFunc_CloseCaption(bf_read &msg)
{
	...
	if ( warnonmissing && !IsX360() )
	{
		...
		if ( !pcheck )
		{
			DevMsg( 2, "No caption found for '%s'\n", tokenname );
		}
	}
...
void OnCaptionLanguageChanged( IConVar *pConVar, const char *pOldString, float flOldValue )
{
	...
	char fn[ 512 ];
	Q_snprintf( fn, sizeof( fn ), "resource/subtitles_%s.txt", var.GetString() );
	...
	if ( Q_strlen( var.GetString() ) > 0 && Q_stricmp( var.GetString(), uilanguage ) )
	{
		if ( !IsX360() )
		{
			...
			{
				char fallback[ 512 ];
				Q_snprintf( fallback, sizeof( fallback ), "resource/subtitles_%s.txt", uilanguage );
		...
		{
			char dbfile [ 512 ];
			Q_snprintf( dbfile, sizeof( dbfile ), "resource/subtitles_%s.dat", var.GetString() );
