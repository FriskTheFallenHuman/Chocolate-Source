...
void COptionsSubVideo::OnApplyChanges()
{
	...
	// windowed
	bool windowed = (m_pWindowed->GetActiveItem() == 1) ? true : false;
	
	// bordeless
	bool bordeless = (m_pWindowed->GetActiveItem() == 2) ? true : false;
	...
	if ( config.m_VideoMode.m_Width != width 
		...
		|| config.NoBorderWindow() != bordeless )
	{
		// set mode
		char szCmd[ 256 ];
		Q_snprintf( szCmd, sizeof( szCmd ), "mat_setvideomode %i %i %i %i\n", width, height, windowed ? 1 : 0, bordeless ? 1 : 0 );
		engine->ClientCmd_Unrestricted( szCmd );
	}
