			...
			(MSurf_TexInfo( surfID )->texinfoFlags & TEXINFO_USING_BASETEXTURE2) )
	{
		...
		if ( pMaterialName )
		{
			...
			if ( bShowIt )
			{
				...
				DevMsg( 2, "Warning: using WorldTwoTextureBlend on a non-displacement surface.\n"
	...
	for ( int i = 0; i < MSurf_VertCount( surfID ); i++ )
	{
			 ...
			 (MSurf_TexInfo( surfID )->texinfoFlags & TEXINFO_USING_BASETEXTURE2) )
		{
			...
			if ( !bWarned )
			{
				const char *pMaterialName = MSurf_TexInfo( surfID )->material->GetName();
				bWarned = true;
				DevMsg( "Warning: WorldTwoTextureBlend found on a non-displacement surface (material: %s). This wastes perf for no benefit.\n", pMaterialName );
			}
