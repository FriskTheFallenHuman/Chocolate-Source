...
void printusage( void )
{
	vprint( 0, "usage:  captioncompiler closecaptionfile.txt\n\
		...
		\ne.g.:  captioncompiler -l subtitles_english.txt" );
...
void PrintHeader()
{
	...
	vprint( 0, "--- Subtitles File compiler ---\n" );
}
...
void CCompileCaptionsApp::CompileCaptionFile( char const *infile, char const *outfile )
{
	...
	for ( StringIndex_t i = g_pVGuiLocalize->GetFirstStringIndex(); i != INVALID_STRING_INDEX; i = g_pVGuiLocalize->GetNextStringIndex( i ), ++c )
	{
		...
		if ( len > blockSize )
		{
			Error( "Subtitle text file '%s' contains a single caption '%s' of %d bytes (%d is max), change MAX_BLOCK_SIZE in captioncompiler.h to fix!!!\n", g_pVGuiLocalize->GetNameByIndex( i ),
